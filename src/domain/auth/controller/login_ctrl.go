...
import (
	...
	"github.com/GeraAnggaraPutra/blueprint-go/src/domain/auth/payload"
	"github.com/GeraAnggaraPutra/blueprint-go/src/domain/auth/repository/model"
	"github.com/GeraAnggaraPutra/blueprint-go/src/handler/jwt"
...
func (c *AuthController) LoginCtrl(ctx echo.Context) error {
	...
	if err != nil {
		return module.ResponseData(ctx, http.StatusBadRequest, nil, err.Error(), "failed create access token")
	}
